              ...
              <div className="lg:col-span-1 bg-white p-6 rounded-xl shadow-md h-fit overflow-y-auto max-h-screen sticky top-4">
                  ...
                  <form onSubmit={onSave} className="space-y-3">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                          ...
                          <input className="p-2 border rounded text-sm" placeholder="Giá/m2 (Tự tính khi xem)" type="number" value={formData.pricePerM2} onChange={e => setFormData({...formData, pricePerM2: e.target.value})} />
                      </div>
                      <div className="grid grid-cols-2 gap-2">
                  ...
                  <table className="w-full text-left border-collapse min-w-[1000px]"> {/* min-w để đảm bảo không bị co rúm trên màn nhỏ */}
                    <thead>
                      <tr className="bg-gray-50 text-gray-600 text-xs uppercase font-bold border-b">
                        ...
                        <th className="p-4 w-[10%]">Kích Thước</th>
                        <th className="p-4 w-[10%]">Hướng</th>
                        <th className="p-4 w-[10%]">Loại</th>
                        <th className="p-4 w-[10%] text-center">Diện Tích</th>
                        <th className="p-4 w-[15%] text-right">Ngày Đăng</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-100">
                      {filteredProperties.map(item => {
                          // TÍNH TOÁN GIÁ/M2: Ép kiểu Number để tránh lỗi phép chia
                          const priceVal = Number(item.totalPrice);
                          const areaVal = Number(item.area);
                          const calculatedPricePerM2 = (areaVal > 0 && priceVal > 0) ? Math.round(priceVal / areaVal) : 0;
                          ...
                          return (
                            <tr key={item._id} className="hover:bg-blue-50 transition-colors group cursor-pointer" onClick={() => { setSelectedProperty(item); setView('detail'); }}>
                                {/* Cột 1: Ảnh + Địa chỉ (Đã bỏ tên chủ nhà) */}
                                <td className="p-4">
                                  <div className="flex items-center gap-4">
                                    <div className="w-16 h-16 rounded-lg overflow-hidden bg-gray-100 border flex-shrink-0">
                                      <img src={item.mainImage || PLACEHOLDER_IMG} className="w-full h-full object-cover" alt="thumb"/>
                                    </div>
                                    <div className="font-bold text-gray-900 text-sm group-hover:text-blue-600 line-clamp-2">
                                      {item.address}
                                    </div>
                                  </div>
                                </td>

                                {/* Cột 2: Giá + Giá/m2 tự tính */}
                                <td className="p-4">
                                  <PriceDisplay value={item.totalPrice} />
                                ...
                                {/* Cột 5: Loại */}
                                <td className="p-4">
                                  <span className="px-3 py-1 rounded-full bg-purple-100 text-purple-700 text-xs font-bold border border-purple-200 whitespace-nowrap">
                                    {item.type}
                                  </span>
                                </td>
